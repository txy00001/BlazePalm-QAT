...
def detect():#####推理
    ...
    model = openvino_executor.compile_model(model = openvino_executor.read_model(model="sim_1.onnx"), device_name="CPU")
        
    palm_detector.load_state_dict(torch.load("weights/BlazePalm_QAT_epoch_120.pth", map_location=torch.device('cpu')))
