...
class ConvTF(quant_nn.QuantConv2d):
...
class ConvPY(nn.Conv2d):
    ...
    def __init__(self, in_channels,
                 ...
                 groups=1,
                 bias=True):
        super(ConvPY, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
        
    def forward(self, input):
        return super(ConvPY, self).forward(input)
...
class FaceMeshBlock(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 3, stride: int = 1):
        ...
        self.conv0 =nn.Conv2d(in_channels=in_channels, out_channels=in_channels, 
...
class NewBlazeBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,padding = (0,0)):
        super(NewBlazeBlock, self).__init__()
        self.stride = stride
        self.channel_pad = out_channels - in_channels
        ...
        self.convs = nn.Sequential(
            ConvPY(in_channels=in_channels, out_channels=in_channels,
                   kernel_size=kernel_size, stride=stride, padding=padding,
                   groups=in_channels, bias=True),
            ConvPY(in_channels=in_channels, out_channels=out_channels,
                   kernel_size=1, stride=1, padding=0, bias=True),
        )
...
class BackBone222(nn.Module):
    def __init__(self):
        super(BackBone222, self).__init__()
        self.backbone1 = nn.ModuleList([
            ConvTF(in_channels = 3, out_channels = 32, kernel_size = (5,5), stride=(2,2)),
            nn.PReLU(32),
            FaceMeshBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1),
            nn.PReLU(32),
            FaceMeshBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1),
            nn.PReLU(32),
            FaceMeshBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1),
            ...
            FaceMeshBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1),
            nn.PReLU(32),
            FaceMeshBlock(in_channels = 32, out_channels = 64, kernel_size = 5,stride = 2),
            nn.PReLU(64),
...
class NewBackBone1(nn.Module):
    def __init__(self):
        super(NewBackBone1, self).__init__()
        self.backbone1 = nn.ModuleList([
            ConvTF(in_channels = 3, out_channels = 32, kernel_size = (5,5), stride=(2,2)),
            nn.PReLU(32),
            NewBlazeBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1,padding=2),
            nn.PReLU(32),
            NewBlazeBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1,padding=(2,2)),
            nn.PReLU(32),
            NewBlazeBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1,padding=(2,2)),
            nn.PReLU(32),
            NewBlazeBlock(in_channels = 32, out_channels = 32, kernel_size = 5,stride = 1,padding=(2,2)),
            nn.PReLU(32),
            NewBlazeBlock(in_channels = 32, out_channels = 64, kernel_size = 5,stride = 2),
            nn.PReLU(64),
            # BlazeBlock(128, 128, 5, 1),
            # nn.PReLU(128),
            # BlazeBlock(128, 128, 5, 1),
