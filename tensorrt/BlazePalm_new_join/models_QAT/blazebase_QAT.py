...
class BlazeLandmark(BlazeBase):
    ...
    def extract_roi(self, frame, xc, yc, theta, scale):
        ...
        for i in range(points.shape[0]):
            ...
            img = cv2.warpAffine(frame, M, (res, res), 1, 0)  # , borderValue=127.5)
            cv2.imshow("demo", img)
...
class BlazeDetector(BlazeBase):
    ...
    def detection2roi(self, detection):
        ...
        if self.detection2roi_method == 'box':
            ...
            # scale = max((detection[:,3] - detection[:,1]),(detection[:,2] - detection[:,0])) # assumes square boxes
    ...
    def detection2roiv1(self, bbox, landmarks):
        # compute box center and scale
        # scale = max(x2, y2) / 2  # assumes square boxes
        # use mediapipe/calculators/util/detections_to_rects_calculator.cc
        ...
        # scale = scale/2
        # compute box rotation
        rot_x0 = landmarks[0][0]
        rot_y0 = landmarks[0][1]
        rot_x1 = landmarks[9][0]
        rot_y1 = landmarks[9][1]
        theta = np.arctan2(rot_y0 - rot_y1, rot_x0 - rot_x1) - np.pi / 2
        # theta = torch.atan2(y0-y1, x0-x1) - self.theta0
        return np.array(xc), np.array(yc), np.array(scale), np.array(theta)
        # return xc, yc, scale, theta
