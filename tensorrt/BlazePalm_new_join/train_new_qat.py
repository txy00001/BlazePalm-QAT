...
parser.add_argument('--training_dataset', default=r'E:\project\qat\handpose_datasets_v2', help='Training dataset directory')
...
parser.add_argument('--resume_net', default=r"E:\project\qat\BlazePalm\weights\blazepalm_Final.pth", help='resume net for retraining')
...
def train(args):
    ...
    for iteration in range(start_iter, max_iter):
        ...
        if iteration % 10== 0:
            export_onnx(net,"quant_model.onnx",1)
            print("save success")
...
def export_onnx(model, onnx_filename, batch_onnx):
    ...
    dummy_input = torch.randn(batch_onnx, 3, 192, 192, device='cuda') 
    torch.onnx.export(model, dummy_input, onnx_filename, verbose=False,training=False, opset_version=opset_version, enable_onnx_checker=False, do_constant_folding=False)
    return True
