...
class QPReLU(QModule):

    def __init__(self, qi=False, num_bits=None):
        super(QPReLU, self).__init__(qi=qi, num_bits=num_bits)
...
class QConvBNReLU(QModule):###Conv2d 和 BN 接收进来，并重新封装成量化的模块
    ...
    def fold_bn(self, mean, std):##bn合并到conv
        if self.bn_module.affine:
            gamma_ = self.bn_module.weight / std####计算gamma
    ...
    def forward(self, x):###合并BN后的forward流程

        if hasattr(self, 'qi'):
            self.qi.update(x)
            x = FakeQuantize.apply(x, self.qi)

        if self.training:###在 forward 的过程更新 BN 的均值、方差
        ...
        self.qw.update(weight.data)#####先获得 BN 层的参数，再把它们 folding 到 Conv 中
