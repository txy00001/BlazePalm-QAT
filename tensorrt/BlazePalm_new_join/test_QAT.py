...
def detect():
    ...
    palm_detector.load_state_dict(torch.load("weights/blazepalm_epoch_310.pth", map_location=torch.device('cpu')))
    ...
    dataset_root = '/home/cmm/PycharmProjects/datas/handpose_datasets_v2'
    ...
    with torch.no_grad():
        ...
        for i in range(len(imgs_path)):
            ...
            for i in range(len(detections)):
                hands = _weighted_non_max_suppression(detections[i])
                hands = torch.stack(hands) if len(hands) > 0 else torch.zeros((0, 18 + 1))
                normalized_palm_detections.append(hands)
