...
class ConvTF2(nn.Conv2d):
    def __init__(self, in_channels,
                 out_channels,
                 kernel_size,
                 stride=1,
                 padding=0,###一般使用same时，stride都是设置为1
                 dilation=1,
                 groups=1,
                 bias=True,
                 ):

        super(ConvTF2, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)

    def forward(self, input):
        return super(ConvTF2, self).forward(input)
...
class BlazePalm_QAT(nn.Module):
    ...
    def forward(self, image, DEPLOY_FLAG=None):
