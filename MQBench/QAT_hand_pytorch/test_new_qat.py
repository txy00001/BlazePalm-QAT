...
def detect():#####推理
    ...
    model = openvino_executor.compile_model(model = openvino_executor.read_model(model="onnx/quant_model.onnx"), device_name="CPU")
        
    #palm_detector.load_state_dict(torch.load(r"E:\pytorch_QAT\QAT_hand_pytorch\weights\blazepalm_Final_cmm.pth", map_location=torch.device('cpu')))
    ...
    #dataset_root = "/opt/data/share/txy/val_hand_v2"
    ...
    with torch.no_grad():
        ...
        for i in range(len(imgs_path)):
            ...
            with torch.no_grad():
                ...
                out_=model([convert_any_to_numpy(img)])###替换原始的model
            ...
            for i in range(len(detections)):
                hand = _weighted_non_max_suppression(detections[i])
                hand = torch.stack(hand) if len(hand) > 0 else torch.zeros((0, 18 + 1))
                normalized_palm_detections.append(hand)
